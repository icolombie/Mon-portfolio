...
const ContactForm = () => {
  ...
  const validate = () => {
    ...
    tempErrors.email = /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email) ? "" : "L'email n'est pas valide.";
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      emailjs.send('service_octi4xm', 'template_9fb0zjs', {
        name,
        email,
        message,
      }, 'PpfZM6pEu9lJT0CjS')
  ...
  return (
    <Container id="contact" maxWidth="lg" className="container-margin-large">
      ...
      <Box component="form" className="form-container" onSubmit={handleSubmit}>
        ...
        {successMessage && (
          <Typography variant="body1" color="green" align="center" className="success-message">
            {successMessage}
          </Typography>
        )}
      </Box>
